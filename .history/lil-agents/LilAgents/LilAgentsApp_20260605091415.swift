...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func setupMenuBar() {
        ...
        let char1Item = NSMenuItem(title: NSLocalizedString("Bruce", comment: "Character name"), action: #selector(toggleChar1), keyEquivalent: "1")
        char1Item.state = .on
        menu.addItem(char1Item)

        let char2Item = NSMenuItem(title: NSLocalizedString("Jazz", comment: "Character name"), action: #selector(toggleChar2), keyEquivalent: "2")
        char2Item.state = .on
        menu.addItem(char2Item)

        menu.addItem(NSMenuItem.separator())

        let soundItem = NSMenuItem(title: NSLocalizedString("Sounds", comment: "Menu item"), action: #selector(toggleSounds(_:)), keyEquivalent: "")
        soundItem.state = .on
        menu.addItem(soundItem)

        // Provider submenu (applies to all characters)
        let providerItem = NSMenuItem(title: NSLocalizedString("Provider", comment: "Menu item"), action: nil, keyEquivalent: "")
        ...
        let gatewayItem = NSMenuItem(title: NSLocalizedString("Advanced Settings\u{2026}", comment: "Menu item"), action: #selector(openGatewaySettings), keyEquivalent: "")
        gatewayItem.tag = -1
        ...
        let sizeItem = NSMenuItem(title: NSLocalizedString("Size", comment: "Menu item"), action: nil, keyEquivalent: "")
        let sizeMenu = NSMenu()
        ...
        // Theme submenu
        let themeItem = NSMenuItem(title: NSLocalizedString("Style", comment: "Menu item"), action: nil, keyEquivalent: "")
        ...
        // Display submenu
        let displayItem = NSMenuItem(title: NSLocalizedString("Display", comment: "Menu item"), action: nil, keyEquivalent: "")
        let displayMenu = NSMenu()
        displayMenu.delegate = self
        let autoItem = NSMenuItem(title: NSLocalizedString("Auto (Main Display)", comment: "Display option"), action: #selector(switchDisplay(_:)), keyEquivalent: "")
        autoItem.tag = -1
        autoItem.state = .on
        ...
        let updateItem = NSMenuItem(title: NSLocalizedString("Check for Updates…", comment: "Menu item"), action: #selector(SPUStandardUpdaterController.checkForUpdates(_:)), keyEquivalent: "")
        updateItem.target = updaterController
        menu.addItem(updateItem)

        menu.addItem(NSMenuItem.separator())

        let quitItem = NSMenuItem(title: NSLocalizedString("Quit", comment: "Menu item"), action: #selector(quitApp), keyEquivalent: "q")
