...
enum AgentProvider: String, CaseIterable {
    ...
    var displayName: String {
        switch self {
        case .claude:   return NSLocalizedString("Claude", comment: "Provider name")
        case .codex:    return NSLocalizedString("Codex", comment: "Provider name")
        case .copilot:  return NSLocalizedString("Copilot", comment: "Provider name")
        case .gemini:   return NSLocalizedString("Gemini", comment: "Provider name")
        case .opencode: return NSLocalizedString("OpenCode", comment: "Provider name")
        case .openclaw: return NSLocalizedString("OpenClaw", comment: "Provider name")
        }
    }

    var inputPlaceholder: String {
        String(format: NSLocalizedString("Ask %@...", comment: "Input placeholder"), displayName)
    }
    ...
    static func detectAvailableProviders(completion: @escaping () -> Void) {
        ...
        for provider in all {
            ...
            if provider == .claude {
                fallbackPaths = [
                    "\(home)/.lil-agents/claude-wrapper.sh",
