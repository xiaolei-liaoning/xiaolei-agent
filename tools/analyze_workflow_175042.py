...
def load_workflow():
    """加载工作流"""
    workflow_path = Path(__file__).parent / "workflows" / "wf_20260421_175042.json"
...
def analyze_workflow(workflow):
    ...
    # 检查连线问题
    if len(workflow.get('edges', [])) == 0:
        issues.append("节点之间没有连线，工作流无法正常执行")
    ...
    if llm_nodes:
        ...
        if not prompt or len(prompt.strip()) == 0:
            issues.append("LLM prompt为空，无法有效处理用户输入")
        if model == 'gpt-4':
...
def generate_optimized_workflow():
    """生成优化后的工作流"""
    optimized_workflow = {
        "id": "wf_20260421_175042_optimized",
        "name": "智能对话工作流（优化版）",
        ...
        "created_at": "2026-04-21T17:50:42.386986",
        "updated_at": "2026-04-21T18:30:00.000000",
        "version": "2.0"
    }
    
    # 保存优化后的工作流
    output_path = Path(__file__).parent / "workflows" / "wf_20260421_175042_optimized.json"
...
def fix_original_workflow():
    ...
    # 保存修复后的工作流
    output_path = Path(__file__).parent / "workflows" / "wf_20260421_175042.json"
    with open(output_path, 'w', encoding='utf-8') as f:
