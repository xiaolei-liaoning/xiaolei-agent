    ...
    class FallbackSkillsConfig:
        def __init__(self):
            ...
            for name, keywords, priority in core_skills:
                self.skills[name] = type('SkillConfigItem', (), {
                    'name': name, 'keywords': keywords, 'priority': priority, 'description': ''
                })()
...
def load_config_from_yaml():
    config_path = "config/skill_keywords.yaml"
...
class SkillDispatcher:
    ...
    def register_tool(self, name: str, keywords: List[str] = None,
        ...
        try:
            ...
            from core.skill_base import LegacySkillAdapter, SkillHandler
            dummy = SkillHandler()
            dummy.name = name
    ...
    def _build_llm_classify_prompt(self, message: str) -> tuple[str, str]:
        ...
        for name, config in self._dynamic_registry.items():
            ...
            kw_str = "、".join(kw[:5])
            lines.append(f"- {name} (关键词: {kw_str}, priority: {config.get('priority', 3)})")
        lines.extend([
            "",
            "规则：",
            "1. 根据用户消息的意图选择最合适的技能",
        ...
        try:
            ...
            try:
                response = loop.run_until_complete(
                    self.llm_router.simple_chat(user_message=user_msg, system_prompt=system_prompt, temperature=0.1)
                )
            finally:
                loop.close()
            ...
            known_skills = {c[0] for c in self.skill_configs} | set(self._dynamic_registry.keys())

            if not skill_name or skill_name not in known_skills:
                logger.debug("LLM 返回无效技能名: %s", skill_name)
                return None, 0.0

            logger.info("LLM 意图分类: message=%r -> skill=%s conf=%.2f reason=%s",
                       message[:60], skill_name, confidence, parsed.get("reason", ""))
    ...
    def match_skill(self, message: str) -> str:
        ...
        if llm_skill is not None and llm_confidence >= 0.6:
            logger.debug("LLM 匹配成功: '%s' -> %s (conf=%.2f)", message[:40], llm_skill, llm_confidence)
            return llm_skill

        if llm_skill is not None:
            logger.debug("LLM 置信度不足: skill=%s conf=%.2f，走规则降级", llm_skill, llm_confidence)
        else:
        ...
        if has_neg and intent_after_neg:
            ...
            if matched_skill != "chat":
                logger.debug("规则降级: 否定处理 '%s' -> %s", message[:40], matched_skill)
        ...
        for intent_name, config in _INTENT_SKILL_MAP.items():
            ...
            if hits >= min_hits:
                logger.debug("规则降级: 意图映射 %s -> %s (hits=%d)", intent_name, skill, hits)
                return skill
    ...
    def _check_mcp_availability(self, message: str) -> Optional[str]:
        ...
        try:
            ...
            # 1. 排除纯聊天场景
            chat_only_keywords = ["你好", "hello", "hi", "hey", "早上好", "晚上好", 
                                 "谢谢", "再见", "bye", "goodbye", "哈哈", "呵呵"]
            if any(kw in message_lower for kw in chat_only_keywords):
                return None
            
            # 2. 检测MCP相关意图（动词+数据源）
            mcp_action_verbs = [
                "查询", "计算", "获取", "连接", "搜索", "分析", "转换", "生成",
                "读取", "写入", "下载", "上传", "同步", "备份", "监控",
                "发送", "接收", "订阅", "发布", "拉取", "推送",
                "query", "calculate", "get", "connect", "search", "analyze"
            ]
            ...
            mcp_data_sources = {
                ...
                "slack": ["slack"],
                "discord": ["discord"],
                "数据库": ["数据库", "database", "sqlite", "postgres", "mysql", "mongodb", "chroma"],
            ...
            if match_score >= 1.0:
                logger.debug(f"MCP候选检测: actions={has_action}, sources={detected_sources}, score={match_score}")
                return "mcp_suggestion"
...
# ── 否定检测 ─────────────────────────────────────────────────────────────
    ...
    def _extract_weather_params(message: str, params: Dict):
        ...
        # 常见非城市词汇黑名单（动词、时间词等）
        non_city_words = {
            "帮我", "给我", "查一下", "看一下", "问一下", "告诉我", 
            "请问", "想要", "需要", "今天", "明天", "后天", "现在",
            "如何", "怎么样", "多少度", "几度", "一下", "天气", "气温",
            "温度", "帮我查", "查一", "看一", "问一"
        }
        ...
        major_cities = {
            ...
            "西安", "天津", "南京", "苏州", "郑州", "长沙", "沈阳", "青岛",
            "宁波", "合肥", "佛山", "昆明", "大连", "厦门", "福州", "济南",
            "哈尔滨", "长春", "石家庄", "南昌", "贵阳", "南宁", "太原",
            "乌鲁木齐", "兰州", "呼和浩特", "银川", "西宁", "拉萨", "海口",
            "三亚", "珠海", "东莞", "中山", "惠州", "江门", "肇庆", "湛江",
            "汕头", "韶关", "梅州", "汕尾", "河源", "阳江", "清远", "潮州",
            "揭阳", "云浮", "无锡", "徐州", "常州", "南通", "连云港", "淮安",
            "盐城", "扬州", "镇江", "泰州", "宿迁", "温州", "嘉兴", "湖州",
            "绍兴", "金华", "衢州", "舟山", "台州", "丽水"
        }
            ...
            if match:
                ...
                if city and city not in non_city_words:
                    ...
                    # 否则，检查是否是合理的城市名（2-4个字，不是常见动词）
                        ...
                        # 额外检查：不包含常见动词
                        verb_indicators = ["帮", "查", "看", "问", "告", "请", "想", "要"]
