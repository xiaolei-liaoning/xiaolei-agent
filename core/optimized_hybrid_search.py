...
class LRUCache:
    ...
    def get(self, query: str, top_k: int, strategy: str,
            vector_weight: float, keyword_weight: float) -> Optional[List[SearchResult]]:
        """获取缓存
...
# 全局优化版混合检索引擎实例
_optimized_hybrid_search_engine = None
_optimized_hybrid_lock = asyncio.Lock()


async def get_optimized_hybrid_search_engine() -> OptimizedHybridSearchEngine:
    """获取优化版混合检索引擎单例
    
    Returns:
        优化版混合检索引擎实例
    """
    global _optimized_hybrid_search_engine
    
    if _optimized_hybrid_search_engine is None:
        async with _optimized_hybrid_lock:
            if _optimized_hybrid_search_engine is None:
                _optimized_hybrid_search_engine = OptimizedHybridSearchEngine()
    
    return _optimized_hybrid_search_engine