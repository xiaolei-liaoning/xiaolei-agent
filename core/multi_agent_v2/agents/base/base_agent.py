...
class AgentFactory:
    ...
    def create_agent(agent_type=AgentType.WORKER, agent_id=None, name=None, description="", **kwargs):
        ...
        return WorkAgent(agent_id=agent_id, name=name or f"agent_{agent_id[:8] if agent_id else '?'}", light_mode=True)
    ...
    def create_agents_for_task(keywords, min_count=2, max_count=5):
        ...
        return [WorkAgent(agent_id=f"agent-{i}", name=f"worker_{i}", light_mode=True) for i in range(count)]
