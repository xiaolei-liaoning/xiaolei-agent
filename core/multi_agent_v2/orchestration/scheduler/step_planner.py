...
6. 工具选择规则：搜索/信息查询→search；读/写/保存文件→file；获取网页内容→fetch_url；执行Python代码→execute_python；执行Shell命令→execute_shell
...
class StepPlanner:
    ...
    async def plan_with_llm(self, task: Task, context: Optional[Dict] = None) -> List[Step]:
        ...
        if context and context.get("available_tools"):
            ...
            # 过滤：只给 LLM 展示搜索/文件/代码执行工具，不展示 call_api 等通用工具
            builtin_names = {"fetch_url", "file", "search", "execute_python", "execute_shell"}
