...
包含：
1. PipelineStrategy - 流水线协作
2. RecursiveTaskDecomposer - 递归任务分解
3. LLM反思机制 - 模型自我评估与迭代优化
4. AdaptivePipelineWithReflection - 带反思机制的自适应Pipeline
"""
    ...
    async def execute(
        ...
        for i, step in enumerate(execution_plan):
            ...
            if not agent:
                ...
                agent_results[agent_id] = ActionResult(
                    success=False, error=f"未找到Agent: {agent_id}"
                )
                break
            ...
            subtask_id, result = await self._execute_agent_task(
                ...
                step_name,
                {
                    ...
                    "complexity": task.complexity / max(len(execution_plan), 1),
