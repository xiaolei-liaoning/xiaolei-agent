...
class MonitoringManager:
    ...
    def __init__(self, interval: int = 5):
        ...
        self._thread = None
        # 日志文件配置
        self.log_file = "./data/monitoring.log"
    ...
    def record_response(self, endpoint: str, status_code: int, duration: float):
        ...
        self._log_event("response", f"Endpoint {endpoint} {status_code} in {duration:.2f}s")
    ...
    def _check_alarms(self):
        ...
        if self.alarm_config["cpu"]["enabled"] and self.metrics["cpu"]:
            recent_cpu = [m["value"] for m in self.metrics["cpu"][-int(self.alarm_config["cpu"]["duration"]/self.interval):]]
            if recent_cpu and all(cpu > self.alarm_config["cpu"]["threshold"] for cpu in recent_cpu):
                if not self.alarm_status["cpu"]:
                    self._trigger_alarm("cpu", f"CPU使用率超过阈值 {self.alarm_config['cpu']['threshold']}%", {
        ...
        if self.alarm_config["memory"]["enabled"] and self.metrics["memory"]:
            recent_memory = [m["value"] for m in self.metrics["memory"][-int(self.alarm_config["memory"]["duration"]/self.interval):]]
            if recent_memory and all(mem > self.alarm_config["memory"]["threshold"] for mem in recent_memory):
                if not self.alarm_status["memory"]:
                    self._trigger_alarm("memory", f"内存使用率超过阈值 {self.alarm_config['memory']['threshold']}%", {
        ...
        if self.alarm_config["disk"]["enabled"] and self.metrics["disk"]:
            ...
            if current_disk > self.alarm_config["disk"]["threshold"]:
                if not self.alarm_status["disk"]:
                    self._trigger_alarm("disk", f"磁盘使用率超过阈值 {self.alarm_config['disk']['threshold']}%", {
        ...
        if self.alarm_config["tasks"]["enabled"] and self.metrics["tasks"]:
            recent_tasks = self.metrics["tasks"][-self.alarm_config["tasks"]["window"]:]
            if len(recent_tasks) >= self.alarm_config["tasks"]["window"]:
                failure_rate = sum(1 for t in recent_tasks if t["status"] == "failed") / len(recent_tasks)
                if failure_rate > self.alarm_config["tasks"]["failure_rate"]:
                    if not self.alarm_status["tasks"]:
                        self._trigger_alarm("tasks", f"任务失败率超过阈值 {self.alarm_config['tasks']['failure_rate']*100}%", {
        ...
        if self.alarm_config["responses"]["enabled"] and self.metrics["responses"]:
            recent_responses = self.metrics["responses"][-self.alarm_config["responses"]["window"]:]
            if len(recent_responses) >= self.alarm_config["responses"]["window"]:
                error_rate = sum(1 for r in recent_responses if r["status_code"] >= 400) / len(recent_responses)
                if error_rate > self.alarm_config["responses"]["error_rate"]:
                    if not self.alarm_status["responses"]:
                        self._trigger_alarm("responses", f"响应错误率超过阈值 {self.alarm_config['responses']['error_rate']*100}%", {
                            "threshold": self.alarm_config["responses"]["error_rate"],
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        # CPU 摘要
        if self.metrics["cpu"]:
            cpu_values = [m["value"] for m in self.metrics["cpu"][-60:]]  # 最近60个数据点
            summary["cpu"] = {
        ...
        if self.metrics["responses"]:
            ...
            success_count = sum(1 for r in recent_responses if 200 <= r["status_code"] < 300)
            error_count = sum(1 for r in recent_responses if r["status_code"] >= 400)
            avg_duration = sum(r["duration"] for r in recent_responses) / len(recent_responses) if recent_responses else 0
    ...
    def save_metrics(self, file_path: str = "./data/metrics.json"):
        """保存指标到文件
    ...
    def load_metrics(self, file_path: str = "./data/metrics.json"):
        """从文件加载指标
    ...
    def configure_alarm(self, alarm_type: str, **kwargs):
        ...
        if alarm_type in self.alarm_config:
            ...
            logger.info(f"告警配置已更新: {alarm_type} = {self.alarm_config[alarm_type]}")
        else:
