...
class BFSTextProcessor:
    ...
    def __init__(self, max_depth: int = 5, max_nodes: int = 100, max_workers: int = 4):
        """初始化BFS处理器
        ...
        logger.info("BFSTextProcessor 初始化完成 (max_depth=%d, max_nodes=%d, max_workers=%d)", 
                   max_depth, max_nodes, max_workers)
    ...
    def extract_context_by_keywords(self, context_queue: deque, 
        ...
        for node in context_queue:
            score = self._calculate_node_score(node, keywords)
            if score > 0:
                scored_nodes.append({
                    "node": node,
                    "score": score,
    ...
    async def async_extract_context_by_keywords(self, context_queue: deque, 
                                               keywords: List[str],
                                               top_k: int = 5) -> List[Dict[str, Any]]:
        """基于关键词从上下文队列中提取相关内容（异步并行版本）
        ...
        logger.info("异步关键词检索完成，返回 %d 个相关节点", len(result))
        return result
    
    def _calculate_node_score(self, node: TextNode, keywords: List[str]) -> float:
        """计算单个节点的相关性分数（用于并行计算）
    ...
    async def async_bfs_traverse(self, root: TextNode) -> deque:
        """异步广度优先遍历内容树，生成上下文队列
        ...
        logger.info("异步BFS遍历完成，生成 %d 个上下文节点", len(result_queue))
        return result_queue
    
    async def async_process_text(self, text: str, 
                                 summarizer: Optional[Callable] = None) -> Dict[str, Any]:
        """完整的异步文本BFS处理流程
        ...
        try:
            ...
            # 步骤3: 异步BFS遍历
            context_queue = await self.async_bfs_traverse(root)
            ...
            logger.info("异步文本BFS处理完成: %d 段落 → %d 上下文节点", 
                       len(paragraphs), len(context_list))
            return result
            
        except Exception as e:
            logger.error("异步文本BFS处理失败: %s", e, exc_info=True)
            return {
                "success": False,
                "error": str(e),
                "paragraphs_count": 0,
    ...
    def shutdown(self):
        ...
        logger.info("BFSTextProcessor 线程池已关闭")
