    ...
    def _check_memory_alert(self):
        ...
        try:
            ...
            if memory_percent > self.alert_threshold:
                ...
                # 发送告警通知
                alert = self.alert_manager.check_alert("memory_percent", memory_percent)
                if alert:
                    import asyncio
            ...
            if current_memory and current_memory > self.memory_threshold:
                ...
                # 发送告警通知
                alert = self.alert_manager.check_alert("memory_used", current_memory)
                if alert:
                    import asyncio
                    asyncio.run(self.alert_manager.send_alert(alert))
        except Exception as e:
            logger.error("检查内存告警失败: %s", e)
