...
# ─── 多步分隔词 ──────────────────────────────────────────────────────────────
_STEP_SEPARATORS = ["先", "然后", "接着", "再", "最后", "之后", "后", "，然后", "，再"]
...
class TaskPlanner:
    ...
    async def process_task(self, task: Dict[str, Any], user_id: int = 1,
    ...
    async def process_task_with_glm(self, task: Dict[str, Any],
        ...
        try:
            ...
            if not router.is_available():
                ...
                return await self.process_task(task, user_id)
    ...
    async def _rule_decompose(self, task: Dict) -> List[Dict]:
        """基于规则的正则拆分多步任务
