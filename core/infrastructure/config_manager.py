...
class SkillsConfig:
    ...
    # 多步任务指示词
    multi_step_indicators: List[str] = field(
    ...
    # 意图到技能的映射
    intent_skill_map: Dict[str, Dict[str, Any]] = field(
        default_factory=lambda: {
            "math_calculation": {
                "keywords": [
                    "加",
...
_EXTERNAL_VECTOR_DB = "/Volumes/xiaaolei/ai-agent-data/vector_db"
_EXTERNAL_KNOWLEDGE_DIR = "/Volumes/xiaaolei/ai-agent-data/knowledge_base"
...
class RAGConfig:
    ...
    knowledge_dir: str = _EXTERNAL_KNOWLEDGE_DIR if os.path.exists("/Volumes/xiaaolei") else "~/.小雷版小龙虾/knowledge_base"
    vector_db_path: str = _EXTERNAL_VECTOR_DB if os.path.exists("/Volumes/xiaaolei") else "~/.小雷版小龙虾/chroma_db"
...
class AppConfig:
    ...
    def _encrypt_value(self, value: str) -> str:
        """简单加密（实际生产应使用专业加密库）"""
        key = (
            self.security.secret_key.encode()
