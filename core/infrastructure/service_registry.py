...
使用方式:
    from .service_registry import ServiceRegistry, initialize_services
...
class ServiceRegistry:
    ...
    def _register_core_services(self, config: AppConfig) -> None:
        ...
        def create_task_processor():
            from ...tasks.concurrent_processor import ConcurrentTaskProcessor
        ...
        pass  # message_bus 服务已移除（agents/message_bus.py 模块已废弃）
        ...
        self.container.register_singleton(
            ISandboxExecutor,
            factory=create_sandbox_executor
        )
        logger.debug("注册服务: ISandboxExecutor")

        def create_clarification_service():
            from ..services.clarification_service import get_clarification_service
            return get_clarification_service()

        self.container.register_singleton(
            IClarificationService,
            factory=create_clarification_service
        )
        logger.debug("注册服务: IClarificationService")
    ...
    def _register_infrastructure_services(self, config: AppConfig) -> None:
        ...
        def create_monitoring_manager():
            from ...monitoring.monitoring import monitoring_manager
