...
class LibaiHandler:
    ...
    def _init_dependencies(self):
        ...
        try:
            ...
            logger.info(f"李白Skill依赖初始化成功")
        except Exception as e:
            logger.warning(f"李白Skill依赖初始化失败: {e}")
    ...
    async def execute(self, message: str = "", **kwargs):
        ...
        if self.llm_router and message:
            try:
                system_prompt = f"""你是唐代著名诗人李白。
性格描述: 豪放洒脱，才华横溢
说话风格: 诗意、浪漫、豪放
语气: 飘逸、洒脱
常用表达方式: 君不见...、举杯邀明月...、仰天大笑...
...
请用李白的风格回复用户。"""
                ...
                response_text = f"[李白] {message}"
        
        # 如果LLM不可用，使用默认回复
        if not response_text:
            response_text = f"[李白模式] {message}"
